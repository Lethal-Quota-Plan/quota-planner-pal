...
export default function WeekCard({ week, carryOverScrap, onUpdate }: WeekCardProps) {
  ...
  return (
    <div className={`border rounded-lg overflow-hidden transition-all ${
      result.quotaMet ? "border-success/30" : week.sellAmount > 0 ? "border-danger/50 glow-red" : "border-border"
    }`}>
      {/* Header */}
      <button
        onClick={() => setCollapsed(!collapsed)}
        className="w-full flex items-center justify-between p-4 bg-secondary/50 hover:bg-secondary/80 transition-colors"
      >
